  ...
  //    for(i=0;i<clickNodeIndex.size();i++){
    ...
    //   }
       ...
       for(i=0;i<clickNodeIndex.size();i++){
    	 ...
    	 double  absoluteLeft=nodeList.get(index2).getMyLeft()-nodeList.get(index2).getMyScrollX();
    	 double  absoluteTop=nodeList.get(index2).getMyTop()-nodeList.get(index2).getMyScrollY();
    	 
    	 for(int j=index2-1;j>=0;j--){
    		 if(nodeList.get(j).getLayer()==layer-1){
    			 absoluteLeft+=nodeList.get(j).getMyLeft()-nodeList.get(j).getMyScrollX();
    			 absoluteTop+=nodeList.get(j).getMyTop()-nodeList.get(j).getMyScrollY();
    			 layer--;
    		 }
    		 if(layer<=0) break;
    	 }	 
